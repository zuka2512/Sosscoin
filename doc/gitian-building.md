...
*Setup instructions for a Gitian build of Sosscoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sosscoin
...
- [Building Sosscoin Core](#building-sosscoin-core)
...
Clone the git repositories for sosscoin and Gitian.
...
in the sosscoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Sosscoin Core
----------------

To build Sosscoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sosscoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sosscoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/sosscoin-project/sosscoin
...
For example:
```bash
URL=https://github.com/thrasher-/sosscoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sosscoin=${COMMIT} --url sosscoin=${URL} ../sosscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sosscoin=${COMMIT} --url sosscoin=${URL} ../sosscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sosscoin=${COMMIT} --url sosscoin=${URL} ../sosscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the sosscoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sosscoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sosscoin
SIGPATH=/some/root/path/sosscoin-detached-sigs

./bin/gbuild --url sosscoin=${BTCPATH},signature=${SIGPATH} ../sosscoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sosscoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sosscoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sosscoin-osx-build.assert
```
...
[sosscoin-project/gitian.sigs.ltc](https://github.com/sosscoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
