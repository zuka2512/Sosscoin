Sosscoin Core version *0.15.1* is now available from:

  <https://download.sosscoin.org/sosscoin-0.15.1/>
...
A number of changes to the way Sosscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 ocjene per kB (0.00001 OSS). This is to prepare for dropping the minimum transaction fee to 0.1 ocjene per kB (0.0001 OSS) in the next release.
